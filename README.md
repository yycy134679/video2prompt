...
批量解析抖音视频链接，调用 AI 模型（Gemini / 火山方舟豆包）进行视频内容理解与审查，结果导出为 Excel，支持一键导入 Lumen 平台。
...
- **Excel 导出**：基于模板导出，包含 pid / 能否翻译 / 提示词 / 抖音链接等字段

## 前置条件

- Python 3.11+
- 抖音解析服务（本地 Docker 部署，默认 `http://localhost:80`）
- API Key：Gemini 或火山方舟（二选一）
...
1. 在左栏分别输入 **pid 列表**和**抖音链接列表**（按行一一对应）
2. 编辑或加载 **提示词**（DEFAULT_USER_PROMPT），可持久化保存
3. 选择**输出格式**：纯文本（保留模型原始输出）或 JSON（结构化解析为"能否翻译 + 信息摘要"）
4. 按需调整**运行时配置**（并发数、帧率、退避序列、熔断阈值等），仅本次运行生效
5. 点击 **开始执行**，实时查看任务进度
6. 执行完成后点击 **导出 Excel** 下载结果
...
- **解析服务**：退避序列 `[10, 30, 120]` 秒，可配置
- **模型服务**：退避序列 `[5, 15, 60]` 秒，可配置
- **全局暂停**：退避期间暂停新任务出队，避免连续失败
