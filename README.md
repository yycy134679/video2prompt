...
- `视频复刻提示词` 默认使用 `docs/视频复刻提示词.md`
- `按类目分析` 默认使用 `docs/视频脚本拆解分析.md`
- 提示词会按模式持久化到 SQLite，并在下次启动时恢复
- `视频复刻提示词` 和 `按类目分析` 支持在界面中恢复各自模式的默认提示词
- 普通模式的输出格式也会持久化并恢复
...
- 公开仓库以工程实现、运行方式和测试结构为主。
- 若后续补充更适合公开展示的架构说明或演示材料，会优先放在 `README.md` 或 `docs/` 下的公开文档中。
